    ...
    <div x-data="{ showAddModal: false, showDetailModal: false, detailData: null, activeTab: 'ao_nuoi' }"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-4">
                    <div class="flex items-center justify-between">
                        <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Hệ thống cảnh báo đỏ tức thời
                        </h3>
                    ...
                    <div class="space-y-2.5">
                        @if(isset($alerts) && count($alerts) > 0)
                            @foreach($alerts as $alert)
                                <div
                                    class="p-3 {{ $alert['type'] === 'danger' ? 'bg-rose-50/60 border-rose-100 text-rose-800' : 'bg-amber-50/60 border-amber-100 text-amber-800' }} border rounded-xl text-xs flex items-start space-x-2.5">
                                    <span class="text-sm">⚠️</span>
                                    <div>
                                        <span class="font-bold">{{ $alert['location'] }}</span>
                                        <p
                                            class="text-[10px] {{ $alert['type'] === 'danger' ? 'text-rose-600' : 'text-amber-600' }} mt-0.5">
                                            {{ $alert['message'] }}</p>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <div
                                class="p-3 bg-emerald-50/60 border border-emerald-100 rounded-xl text-emerald-800 text-xs flex items-start space-x-2.5">
                                <span class="text-sm">✅</span>
                                <div>
                                    <span class="font-bold">Mọi chỉ số đều ổn định</span>
                                    <p class="text-[10px] text-emerald-600 mt-0.5">Không có cảnh báo nào trong thời gian gần
                                        đây.</p>
                                </div>
                            </div>
                        @endif
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-3">
                    <div class="flex items-center justify-between">
                        <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Biểu đồ so sánh nhiệt độ nước
                            vs nhiệt độ không khí</h3>
        ...
        @if($title === 'Chi phí vận hành')
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-4">
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="p-3 bg-slate-50 rounded-xl border border-slate-150">
                            ...
                            <span class="text-[10px] text-slate-400 mt-0.5 block">100% chi phí được ghi nhận trực tiếp cho 1
                                ao nuôi hoặc khu nuôi cụ thể.</span>
                        </div>
                        <div class="p-3 bg-slate-50 rounded-xl border border-slate-150">
                            <span class="font-bold text-slate-700 block">Phân bổ chia đều (Equal Split)</span>
                            <span class="text-[10px] text-slate-400 mt-0.5 block">Chi phí chung (điện, nước sinh hoạt) được
                                hệ thống tự động chia đều theo diện tích hoặc số ao hoạt động.</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-3">
                    <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Cơ cấu phân bố chi phí vận hành
                    </h3>
                    <div class="relative h-44">
        ...
        @if($title === 'Vật tư & Kho')
            <!-- Material Warnings -->
            <div
                class="bg-amber-50 border border-amber-200/60 rounded-2xl p-4 flex items-start space-x-3 text-xs text-amber-800">
                <span class="text-base">⚠️</span>
                <div>
                    <span class="font-bold">Cảnh báo tồn kho dưới hạn mức tối thiểu</span>
                    <p class="text-[10px] text-amber-600 mt-0.5">Hiện tại có 2 vật tư đã xuống dưới định mức tối thiểu hoặc
                        hết hàng (Men vi sinh BioPro, Khoáng bột AquaMineral). Vui lòng lập phiếu đề xuất nhập kho bổ sung.
                    </p>
                </div>
            </div>
        @endif
        ...
        @if($title === 'Quản lý vụ nuôi')
            ...
            <!-- <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-sm space-y-4">
                    <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Quy trình lộ trình vụ nuôi tiêu chuẩn
                    </h3>
                    <div
                        class="relative flex items-center justify-between text-slate-400 text-[10px] font-bold uppercase tracking-wider">
                        ...
                        <div class="flex flex-col items-center space-y-1 bg-white px-2">
                            <span
                                class="w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center">1</span>
                            <span>Khởi tạo</span>
                        </div>
                        <div class="flex flex-col items-center space-y-1 bg-white px-2">
                            <span
                                class="w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center">2</span>
                            <span>Cải tạo ao</span>
                        </div>
                        <div class="flex flex-col items-center space-y-1 bg-white px-2">
                            <span
                                class="w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center">3</span>
                            <span>Thả giống</span>
                        </div>
                        <div class="flex flex-col items-center space-y-1 bg-white px-2 text-indigo-600">
                            <span
                                class="w-6 h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center">4</span>
                            <span class="font-black">Đang nuôi</span>
                        </div>
                        <div class="flex flex-col items-center space-y-1 bg-white px-2">
                            <span
                                class="w-6 h-6 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center">5</span>
                            <span>Thu hoạch</span>
                        </div>
                        <div class="flex flex-col items-center space-y-1 bg-white px-2">
                            <span
                                class="w-6 h-6 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center">6</span>
                            <span>Kết thúc</span>
                        </div>
                    </div>
                </div> -->
