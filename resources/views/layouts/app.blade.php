...
<body
    x-data="{ showLogoutConfirm: false, sidebarCollapsed: localStorage.getItem('sidebarCollapsed') === 'true', mobileSidebarOpen: false }"
    class="font-sans antialiased bg-slate-50/50">
    <div class="h-screen flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
            <!-- Top Header -->
            <header
                class="bg-white border-b border-slate-100 px-4 md:px-8 py-4 flex items-center justify-between shrink-0 shadow-sm">
                <div class="flex items-center space-x-3">
                    ...
                    <span class="text-xs font-bold text-slate-400 uppercase tracking-widest hidden sm:inline">Hệ thống
                        giám sát</span>
                    <span class="text-slate-300 hidden sm:inline">|</span>
                    <span
                        class="text-xs font-bold text-emerald-600 uppercase tracking-wider bg-emerald-50 px-2 py-0.5 rounded">Trang
                        trại đang hoạt động</span>
                </div>

                <div class="flex items-center space-x-4">
                    <!-- Profile Edit Link -->
                    <a href="{{ route('profile.edit') }}"
                        class="flex items-center space-x-2 text-xs font-bold uppercase tracking-wider text-slate-700 hover:text-slate-950 bg-slate-50 border border-slate-200 px-3 md:px-4 py-2 rounded-lg hover:bg-slate-100 transition-all">
            ...
            <main class="flex-1 overflow-y-auto overflow-x-hidden bg-slate-50/50 p-4 md:p-8">
                ...
                @if (session('success') || session('error') || $errors->any())
                    <div class="max-w-7xl mx-auto mb-6">
                        @if (session('success'))
                            <div
                                class="p-4 mb-4 text-sm text-emerald-800 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center space-x-2">
                        ...
                        @if (session('error'))
                            <div
                                class="p-4 mb-4 text-sm text-red-800 rounded-2xl bg-red-50 border border-red-100 flex items-center space-x-2">
